...
class Print(Step):
    ...
    def run(self, input: pd.DataFrame):
        ...
        # Columns order
        columns = ['Name', 'Ticker', 'Change', 'Price',
                   'Actual', 'Before', 'After', 'Target', 'Group']
        input_columns = set(input.columns)
        columns = [
            column for column in columns if column in input_columns]
        ...
        if set(percentage_columns).issubset(input_columns):
            formatted[percentage_columns] = (
                formatted[percentage_columns] * 100)
            formatted[percentage_columns] = formatted[percentage_columns].round()
            formatted[percentage_columns] = formatted[percentage_columns].astype(
                int)
